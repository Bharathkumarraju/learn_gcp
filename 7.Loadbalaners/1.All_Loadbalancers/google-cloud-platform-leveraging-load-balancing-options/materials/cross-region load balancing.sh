...
Set the Name to spikey-us-webserver1
...
Click Management and set the Startup script to the following
	...
	echo '<!doctype html><html><body><h1>spikey-us-webserver1</h1></body></html>' | sudo tee /var/www/html/index.html
...
gcloud compute instances create spikey-us-webserver2 \
...
Click Create instance.
Set the Name to spikey-europe-webserver1
...
Click Management and set the Startup script to the following
	...
	sudo a2enmod ssl
	sudo service apache2 restart
	echo '<!doctype html><html><body><h1>spikey-europe-server1</h1></body></html>' | sudo tee /var/www/html/index.html

#create the fourt instance for europe zone using gcloud
 Go to the spikey-europe-webserver1
 Click on the CREATE SIMILAR option
 Change the name to spikey-europe-webserver2
Click Management, Security, Disks, Networking, Sole Tenancy to to reveal advanced settings
Change the startup script by just change the page name to spikey-europe-webserver2 leave remaining thing as it is
...
Set the Name to spikey-external-traffic
...
#using gcloud
gcloud compute firewall-rules create spikey-external-traffic \
...
Set the Name to spikey-us-instancegroup for HTTP
Set the Zone to us-central1-a.
...
spikey-us-webserver-1 and create spikey-us-webserver-2
Leave the other settings as they are.
Click Create.


#Create an instance group for each of your zones.(using gcloud)
gcloud compute instance-groups unmanaged create spikey-europe-instancegroup --zone europe-west1-b

#Add the instances you created earlier to the instance groups.

gcloud compute instance-groups unmanaged add-instances spikey-europe-instancegroup \
    --instances spikey-europe-webserver1,spikey-europe-webserver2 \
    --zone europe-west1-b
...
For the Name of the load balancer, enter spikey-web-map.
...
Set the Name of the backend service to spikey-web-map-backend-service.
#Set the Protocol.
For HTTP protocol, leave the values set to the defaults.
Under Backends, set Instance group to spikey-us-instancegroup
#Set the port number.
For HTTP traffic between the load balancer and the instances, leave Port numbers at 80.
Leave the default values for rest of the fields.
Click Add backend.
Select the spikey-europe-instancegroup
For HTTP traffic between the load balancer and the instances, leave Port numbers at 80.
Leave the default values for rest of the fields.
Click Done.
Under Health check, select Create a health check or Create another health check.
	a)If you are creating an HTTP health check, set the following health check parameters:
		Name to spikey-http-basic-check
		Protocol to HTTP
		Port to 80
...
Set Name to spikey-http-frontend-rule.
Set the Protocol.
	Select HTTP if you want HTTP between the client and the load balancer.
Set IP version to IPv4.
In the IP address field, select spikey-static-external-ipv4, which you created earlier.
...
Click on load balancer named spikey-web-map to expand the load balancer you just created.
