...
gcloud compute networks create bharath-custom-network --subnet-mode custom

#Create a new subnet in your custom VPC network
gcloud compute networks subnets create bharath-custom-subnet \
    --network bharath-custom-network \
    --range 10.128.0.0/20 \
    --region us-central1
...
Enter a Name of bharath-custom-network
Under subnets, enter a Name of bharath-custom-subnet
...
Set Network to bharath-custom-subnet
...
gcloud compute instances create bharath-us-central-server1 \
        --image-family debian-9 \
        --image-project debian-cloud \
        --tags internal-loadbalancer \
        --zone us-central1-b \
        --subnet bharath-custom-subnet \
        --metadata startup-script="#! /bin/bash
          ...
          echo '<!doctype html><html><body><h1>bharath-us-central-server1</h1></body></html>' \
          | tee /var/www/html/index.html
          EOF"

gcloud compute instances create bharath-us-central-server2 \
    --image-family debian-9 \
    --image-project debian-cloud \
    --tags internal-loadbalancer\
    --zone us-central1-b \
    ...
    --metadata startup-script="#! /bin/bash
      ...
      a2enmod ssl
      service apache2 restart
      echo '<!doctype html><html><body><h1>bharath-us-central-server2</h1></body></html>' | tee /var/www/html/index.html
      EOF"

gcloud compute instances create bharath-us-central-server3 \
    --image-family debian-9 \
    --image-project debian-cloud \
    --tags internal-loadbalancer \
    --zone us-central1-c \
    ...
    --metadata startup-script="#! /bin/bash
      ...
      a2enmod ssl
      service apache2 restart
      echo '<!doctype html><html><body><h1>bharath-us-central-server3</h1></body></html>' | tee /var/www/html/index.html
      EOF"

gcloud compute instances create bharath-us-central-server4 \
    --image-family debian-9 \
    --image-project debian-cloud \
    --tags internal-loadbalancer \
    --zone us-central1-c \
    ...
    --metadata startup-script="#! /bin/bash
      ...
      a2enmod ssl
      service apache2 restart
      echo '<!doctype html><html><body><h1>bharath-us-central-server4</h1></body></html>' | tee /var/www/html/index.html
      EOF"
...
Set Name to bharath-us-server1
...
Under Networking, ​set Network to my-custom-network and Subnetwork to bharath-custom-subnet
...
#Create bharath-us-central-server2 with the same settings, except with Startup script set to
...
#Create bharath-us-central-server3 with the same settings, except with except with Zone set to us-central1-c and Startup script set to
sudo apt-get update
sudo apt-get install apache2 -y
sudo a2ensite default-ssl
sudo a2enmod ssl
sudo service apache2 restart
echo '<!doctype html><html><body><h1>bharath-us-central-server3</h1></body></html>' | sudo tee /var/www/html/index.html

#Create bharath-us-central-server4 with the same settings, except with except with Zone set to us-central1-c and Startup script set to
...
sudo a2enmod ssl
sudo service apache2 restart
echo '<!doctype html><html><body><h1>bharath-us-central-server4</h1></body></html>' | sudo tee /var/www/html/index.html
...
gcloud compute instance-groups unmanaged add-instances bharath-us-instancegroup1 \
    --instances bharath-us-central-server1,bharath-us-central-server2 --zone us-central1-b
...
gcloud compute instance-groups unmanaged add-instances bharath-us-instancegroup2 \
    --instances bharath-us-central-server3,bharath-us-central-server4 \
    --zone us-central1-c
...
Set the Name to bharath-us-instancegroup1
Set the Zone to us-central1-b.
Under Group type, select Unmanaged instance group.
Set Network to  bharath- bharath-custom-network
Set Subnetwork to bharath-custom-subnet
From VM instances select bharath-us-central-server1
...
gcloud compute backend-services add-backend bharath-internal-lb \
...
gcloud beta compute forwarding-rules create bharath-internal-lb-forwarding-rule \
    --load-balancing-scheme internal \
    --address 10.128.0.6 \
    --ports 80 \
    --network bharath-custom-network \
    --subnet bharath-custom-subnet \
    --service-label bharath-ilb-label \
    --region us-central1 \
    --backend-service bharath-internal-lb
...
Click Continue
Set the Name to bharath-internal-lb.
#Configure backend services
Click Backend configuration.
The Name of the backend service appears as bharath-internal-lb
Set Region to us-central1
Set Network to bharath-custom-network
Under Backends, select instance group bharath-us-instancegroup1
Click Add backend
Select instance group bharath-us-instancegroup2
...
Under Subnetwork, select bharath-custom-subnet
...
##Configure another firewall rule to allow health check probes from the health checker.
Click Create firewall rule.
Set Name to bharath-allow-health-check
Set VPC network  to bharath-custom-network
Under Targets, select Specified target tags.
...
gcloud compute instances create bharath-standalone-instance \
    --image-family debian-9 \
    --image-project debian-cloud \
    --zone us-central1-b \
    --tags standalone \
...
Go to the VM instances page in the Google Cloud Platform Console. 
Click Create instance.
Set Name to bharath-standalone-instance1.
Set the Zone to us-central1-b.
...
Under Networking, edit the network interface. Set Network to bharath-custom-network and Subnetwork to bharath-custom-subnet
Click Create.


#Delete the external IP addresses from your instances(From gcloud)

gcloud compute instances delete-access-config bharath-us-central1-1 \
    --access-config-name = "External NAT" --zone us-central1-b

gcloud compute instances delete-access-config bharath-us-central1-2 \
    --access-config-name = "External NAT" --zone us-central1-b

gcloud compute instances delete-access-config bharath-us-central1-3 \
    --access-config-name = "External NAT" --zone us-central1-c

gcloud compute instances delete-access-config bharath-us-central1-4 \
    --access-config-name = "External NAT" --zone us-central1-c

#Delete the external IP addresses from your instances(from Console)
Go to the VM instances page in the Google Cloud Platform Console
Click on bharath-us-central-server1
Click Edit.
...
gcloud compute --project bharath-lb ssh --zone us-central1-b bharath-standalone-instance
